...
\begin{document}
	...
	\input{prefix/statutorydeclaration.tex}
	\input{prefix/genderclause.tex}
...
\chapter{Grundlagen}\label{ch:Grundlagen}
	...
	\section{Weitere Technologien}
		...
		Das \ac{MQTT} ist ein Nachrichtenprotokoll, dass sich durch seine robusten Eigenschaften vor allem für unzuverlässige Umgebungen eignet. Es ist entwickelt worden um mit Geräten mit hoher Latenz, geringer Bandbreite oder labilem Netzwerk zu kommunizieren. Es eignet sich vor allem für \ac{M2M}-Kommunikation und den \ac{IoT} Bereich, da es besonders Akku- und Leistungssparen ist. \cite{krohn_was_2022}
		
		\ac{MQTT} baut auf einer Subscribe and Publish Architektur auf. Ein Eingabegerät, beispielsweise ein Temperatursensor, erhebt seine Daten und reicht sie an einen MQTT Broker weiter, welcher die Daten veröffentlicht (Publish). Möchte ein Client auf die Daten zugreifen, muss er den korrekten \textit{Topic} unter dem die Daten liegen abonnieren und erhält daraufhin alle Änderungen des Datensatzes (Subscribe). \cite{krohn_was_2022}
		
		Der \ac{MQTT} Broker übernimmt das Versenden der Daten an alle Clients. Für einen Client sehen die Topics nach einer hierarchischen Struktur aus. Sollte die Verbindung zu einem Client unterbrochen werden, puffert der Broker die Veränderungen an den Daten und sendet diese, wenn der Client wieder verfügbar ist. \cite{krohn_was_2022}
		
		% QoS und Nachricht
		Möchte ein MQTT-Client Daten veröffentlichen, sendet er eine Nachricht an den Broker. Diese Nachricht besteht aus Topic und Payload. Das Topic legt fest, unter welchem Namen andere Clients die Daten finden können, während die Payload die Daten beinhaltet. Diese müssen keiner festen Struktur folgen und können vom Client selbst strukturiert werden. Allerdings macht es Sinn, die Daten in standardisierten Formaten wie XML oder JSON zu verfassen, um das Auslesen zu vereinfachen. Außerdem legt der Client fest, welche Quality of Service er einhalten möchte. Es gibt drei Abstufungen. Auf Stufe 0, der niedrigsten Stufe, werden die Daten einmalig versendet, egal ob der Broker sie erhält oder nicht. Auf der ersten Stufe wird sichergestellt, dass der Broker die Daten bekommt, indem der Client die Daten so oft wiederholt sendet, bis der Broker mit einer Empfangsbestätigung antwortet. Dies kann dazu führen, dass der Broker mit vielen Anfragen überschwemmt wird und er die Nachrichten öfter speichert. Auf der höchsten Stufe wird dieses Problem behoben. Der Client sendet nur eine Nachricht, stellt jedoch sicher, dass der Broker sie erhält. Diese Art der Kommunikation benötigt mehr Bandbreite als Quality of Service 0 und 1. \cite{krohn_was_2022}
		
		% Sicherheit
		MQTT gilt als sicheres Protokoll. Um Nachrichten zu verschlüsseln, wird TLS-\linebreak Verschlüsselung angewandt. Außerdem werden Authentifizierungen für die Clients eingerichtet, um sicherzustellen, dass nur berechtigte Teilnehmer Daten schreiben und lesen können. Zusätzlich können Administratoren des MQTT-Brokers VPNs und Firewalls verwenden, um die Sicherheit des Systems weiter zu stärken. \cite{noauthor_was_2023}
...
\chapter{Analyse und Entwurf}\label{ch:Ergebnisse}
	...
	\section{Anforderungsanalyse}
		...
		% Ziel
		Die Daten, welche die Simulation der Maschine generiert, sollen an eine Grafana Instanz gesendet werden, da Grafana das bevorzugte Visualisierungstool bei AZO ist. Außerdem bietet Grafana durch die leichte Konfiguration eine schnelle und komfortable Möglichkeit, Daten visuell darzustellen. Hier soll erneut ein umatifähiger OPC UA Server verwendet werden. Die Daten sollen in der grafischen Oberfläche von Grafana einfach und umfangreich ausgelesen und angezeigt werden können. Das Ziel ist es, einen Ablauf zu entwerfen, welcher als Ansatzpunkt für zukünftige Projekte verwendet werden kann. 
...
\chapter{Implementierung}\label{ch:Implementierung}
	...
	\section{Implementierung eines internen Prototyps}\label{ch:Implementierung-Intern}
		...
		Um die Daten aus dem MQTT Broker zu lesen und in die InfluxDB zu schreiben, können zwei Methoden verwendet werden. NodeRed ist eine Low-Code-Plattform, mit der grafisch Datenflüsse programmiert werden können. Es verwendet Codeblocks, um Anweisungen zu speichern. Telegraf ist ein Open-Source-Server-Agent und Datenverarbeitungsframework, das von der Firma InfluxData entwickelt wurde. Es wurde speziell für die Erfassung, Verarbeitung und Übertragung von Metriken und Ereignisdaten entwickelt und ist ein wichtiger Bestandteil des InfluxData-Ökosystems. 
			...
			\begin{itemize}
				...
				\item \textbf{Topic}: umati/v2/rolog/ControllerType/nsu=http:\_2F\_2Fvdma.org\_2FOPC\linebreak RoboticsTestServer\_2F;i=5002
	...
	Die Ausarbeitung der Funktionalen Anforderungen sowie die Evaluation der Prototypen findet in Kapitel \ref{Prototyp:1} und \ref{Prototyp:2} statt. Die Flyer, welche in Anhang III einsehbar sind, erfüllen die Anforderungen für anschauliche informative Flugblätter zur Aufklärung über den neuen Standard. Der erste Flyer ist dabei eine mehr AZO orientierte Darstellung in Orange, der Zweite behandelt Umati als Community und ist im Blau-Rosa des Standards gefärbt.
	
		
	\chapter{Erprobung}
	
	% Beschreibung Aufbau Dashboard
	
	Das Dashboard besteht aus zwei Sektionen. Zunächst werden die Daten für die Achsen und im Anschluss wird die Motortemperatur ausgegeben. Für jede Variable werden einmal alle Werte der letzten 5 Minuten in einem Diagramm und der aktuellste Wert in einem Textfeld angezeigt. In Abbildung \ref{fig:Dashboard1} sind die Achsendaten der ersten Achse dargestellt. Die Daten liegen in der oberen Row, wobei die Daten der zweiten Achse eingeklappt sind. Am oberen linken Rand kann die Variable angepasst werden, um nur die gewünschten Achsen anzeigen zu lassen. Dies funktioniert wie zu erwarten und es kann zwischen Achse 1, Achse 2 oder beiden ausgewählt werden. An der Oberen Rechten Ecke kann das Aktualisierungsintervall des Dashboards angepasst werden, wobei 5 Sekunden der kleinstmögliche Wert ist. 
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{res/Dashboard/Dashboard_1.png}
		\caption{Diagramme der Achsendaten auf dem finalen Dashboard} 
		\label{fig:Dashboard1}
	\end{figure}
	
	% Beschreibeung Dashboard Aufbau
	Unter den Achsendaten wird wie in Abbildung \ref{fig:Dashboard2} die Motortemperatur ausgegeben. Das Diagramm und die Aktuelle Anzeige sind identisch mit den Variablen der Achsen konzipiert. In den beiden Abbildungen \ref{fig:Dashboard1} und \ref{fig:Dashboard2} ist das Dashboard mit den von Node-Red gelieferten Daten abgebildet. Das Dashboard, welches die Daten über Telegraf einliest, ist identisch aufgebaut und unterscheidet sich nur anhand der Abfragen an die InfluxDB.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{res/Dashboard/Dashboard_2.png}
		\caption{Diagramme der Motortemperatur auf dem finalen Dashboard} 
		\label{fig:Dashboard2}
	\end{figure}
	
	% Vergleich Node Red und Telegraf
	Der einzige Unterschied zwischen den beiden Dashboards liegt in der Datenbankabfrage und der Komplexität des Abfragebefehls. Bei den NodeRed-Daten gestaltet sich das Auslesen extrem einfach und kompakt, da die Daten bereits im gewünschten Format vorliegen. Meistens ist die Abfrage auch im Grafana-Baukasten einfach zu erstellen, da es nur wenige, leicht lesbare Variablennamen gibt. Die Auswahl der gewünschten Daten erfolgt bereits in NodeRed, bevor sie in der Datenbank gespeichert werden. 
	
	Bei Telegraf gestaltet sich die Abfrage bei Grafana etwas komplizierter. Telegraf verwendet den Pfad der Daten in der JSON-Datei als Spaltennamen. Dies führt insbesondere bei komplexen JSON-Dateien zu langen Namen, die in Grafana ausgewählt werden müssen. Telegraf wandelt alle Felder der JSON, die Werte sein können, in eigene Spalten in der Datenbank um. Dadurch entstehen viele Datenpunkte, auf die Grafana zugreifen kann. Die Daten werden in Grafana transformiert, was zu längeren und schwerer lesbaren Abfragen führt. 
	
	Allerdings kann dies auch Vorteile mit sich bringen. Da alle Daten in Grafana vorliegen, können schnell neue Diagramme erstellt werden. Beispielsweise können leicht Maßeinheiten ausgelesen und angezeigt werden. Bei der Lösung mit NodeRed muss hierfür der neue Wert zunächst transformiert und dann aus der InfluxDB ausgelesen werden.
	
	Beide Umsetzungen haben ihre Vor- und Nachteile, die je nach Situation angewendet werden können. Ein weiterer Faktor bei der Entscheidung ist die bestehende Infrastruktur. Falls bereits ein NodeRed-Server existiert, der viele Daten aus anderen Projekten verarbeitet, könnte es einfacher sein, auch Umati-Daten über dieselbe Plattform zu bewegen. Ist es jedoch ein eigenständiges System, ist Telegraf als offizielles InfluxData-Projekt möglicherweise die bessere Lösung.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[height=0.9\textheight, page=1]{res/Flyer/Flyer.pdf}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[height=0.9\textheight, page=2]{res/Flyer/Flyer.pdf}
	\end{figure}
	\pagebreak
